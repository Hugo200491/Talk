...
const App = ({route, navigation}) => {
  ...
  async function AddCommentCantine() {
    ...
    const addcomment = db.collection('prof_comments');
  ...
  async function AddLike(itemmm) {
    ...
    const cherche = db.collection(item+"_likeprof");
    ...
    const like = db.collection("prof_comments").doc(itemmm["name_id"]);
    await like.update({like: itemmm["like"]+1});

    const newlike = db.collection(item+"_likeprof");
